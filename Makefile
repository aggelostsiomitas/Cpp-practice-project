#compiler and flags 
CXX=g++  
CXXFLAGS= -Iinc -Wall -Wextra -std=c++17 #Wall and Wextra are for warnings,-Inc is for includeing files form inc directory , c++17 is
...
$(OBJ_DIR)/%.o: $(SRC_DIR)/%.cpp
	@mkdir -p $(OBJ_DIR)
    @echo "Compiling $< ..."
    $(CXX) $(CXXFLAGS) -c $< -o $@  # $< is for source file , $@ is for target file , the .cpp files  becomes .o
...
test: $(TEST_DIR)/test_matrix.cpp $(LIB)
    ...
    $(CXX) $(CXXFLAGS) -L$(LIB_DIR) -lmatrix $< -o $(TEST_DIR)/test_matrix

# Demo target
demo: $(EXAMPLE_DIR)/demo.cpp $(LIB)
    @echo "Building demo executable..."
    $(CXX) $(CXXFLAGS) -L$(LIB_DIR) -lmatrix $< -o $(EXAMPLE_DIR)/dem

# Clean target
clean:
    @echo "Cleaning up..."
    rm -f $(OBJ_DIR)/*.o $(LIB) $(TEST_DIR)/test_matrix $(EXAMPLE_DIR)/demo
